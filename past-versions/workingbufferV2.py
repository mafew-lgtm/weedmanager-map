...
<head>
    ...
    <style>
        ...
        * {{ box-sizing: border-box; }}
        ...
        .hotspot-label {{ flex: 1; }}
        .hotspot-count {{ color: #666; white-space: nowrap; }}
        .view-btn {{
...
<script>
    ...
    // Approximate ~300m grid cells (0.003 degrees). This is a coarse heuristic,
    // not a true geodesic clustering — good enough to flag "go look here" spots.
    const HOTSPOT_CELL_DEG = 0.003;
    const HOTSPOT_TOP_N_PER_SPECIES = 3;
    ...
    function computeHotspots() {{
        ...
        (geoData.points.features || []).forEach(f => {{
            ...
            coordsList.forEach(coords => {{
                ...
                if (!cellsBySpecies[spName][key]) {{
                    cellsBySpecies[spName][key] = {{ count: 0, latSum: 0, lngSum: 0 }};
                }}
                const cell = cellsBySpecies[spName][key];
                cell.count++;
                cell.latSum += lat;
                cell.lngSum += lng;
            }});
        ...
        Object.keys(cellsBySpecies).forEach(spName => {{
            const topCells = Object.values(cellsBySpecies[spName])
                .filter(c => c.count >= HOTSPOT_MIN_POINTS)
                .sort((a, b) => b.count - a.count)
                .slice(0, HOTSPOT_TOP_N_PER_SPECIES);
    ...
    function renderHotspotsTab() {{
        ...
        if (hotspots.length === 0) {{
            container.innerHTML = '<div class="empty-msg">No dense clusters detected (~300m grid, min 2 pts).</div>';
            return;
        }}

        let html = '<div class="hotspot-note">Densest ~300m clusters per species</div>';
        hotspots.forEach((h, i) => {{
            html += `<div class="hotspot-row" data-idx="${{i}}">
                <span class="badge" style="background:${{colorFor(h.species)}}"></span>
                <span class="hotspot-label">${{h.species}}</span>
                <span class="hotspot-count">${{h.count}} pts</span>
        ...
        container.querySelectorAll('.view-btn').forEach(btn => {{
            btn.addEventListener('click', () => {{
                ...
                L.popup()
                    ...
                    .setContent(`<b>${{h.species}}</b> hotspot — ${{h.count}} points nearby`)
                    .openOn(map);
            }});
        }});
    }}
